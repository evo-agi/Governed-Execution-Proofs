...
Current Temporal dev-server DB placement guidance is also carried by
`.docs/governed-lifecycle-proof-2026-05-28.md`. Any future Temporal dev-server
command in this older plan must use a run-scoped or timestamp-scoped DB path such
as `.out/temporal/<run-id-or-timestamp>-dev-server.db`.

Do not use `.out/temporal/dev-server.db`; reusable DB paths risk stale or shared
Temporal dev-server state across rehearsals. Do not use
`.out/<run_id>/temporal/dev-server.db`; CP validation owns the run-root
lifecycle and may delete or recreate `.out/<run_id>/`, which can remove or
invalidate nested Temporal state. This is proof-harness reproducibility guidance
only, not a production Temporal persistence architecture, public/demo readiness,
HA, multi-tenancy, or general deployment architecture claim.
...
| `temporal server start-dev --db-filename .out/temporal/<run-id-or-timestamp>-dev-server.db` | `Control Plane: README.md:150-161`; `Control Plane: .docs/architecture.md:171` | CP Temporal normal-shell proof | No | Yes, run-scoped Temporal DB under `.out/temporal/` | No | Temporal DB/logs | Temporal dev tooling, CP config, unique run-scoped DB path | Local Temporal availability for the selected rehearsal | Remote ES proof, production Temporal persistence, HA, multi-tenancy | high | required |
...
| 4. CP Temporal normal-shell proof invocation | `python ../fake_model_server.py --port 18081`; `temporal server start-dev --db-filename .out/temporal/<run-id-or-timestamp>-dev-server.db`; `run-temporal-shell-worker --config config.yaml`; `python ../supported_local_validation_driver.py --mode temporal-basic ...` | Worker/Temporal unavailable, Temporal DB state reused or nested under the CP validation run root, stage blocker, missing provider run id, missing terminal CP outcome | CP validation summary, attempt manifests, Temporal summary, provider refs | One bounded CP Temporal normal-shell proof mode | Not all modes, not production, not default remote runtime | Use a unique `.out/temporal/<run-id-or-timestamp>-dev-server.db`; do not use `.out/temporal/dev-server.db` or `.out/<run_id>/temporal/dev-server.db`; do not inspect raw prompts/model outputs/workspaces |
